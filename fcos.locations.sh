...
locations=(
	...
	"$commandDir/layer/groan/groan/groan.commands" # merge with groan sub-commands 
)

defaultDispatch="_dispatch.sh"
defaultSubSubcommand="default"

themePath=( "$commandDir/layerbox/groan/groan/groan.themes.sh" "$commandDir/$commandName.themes.sh" "$commandDir/$commandName.theme-$THEME.sh" )
